...
export default function TaskInsightPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black text-zinc-200 animate-pulse uppercase tracking-[0.4em]">Decoding Node...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] py-8 px-6 font-sans selection:bg-indigo-100">
      <div className="max-w-6xl mx-auto">
        ...
        {/* 2. CONSOLIDATED HEADER & METRICS */}
        <div className="bg-white border-2 border-slate-100 p-8 rounded-[2.5rem] shadow-xl shadow-indigo-500/5 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="flex gap-6 bg-[#f8f9fc] p-5 rounded-3xl border border-slate-100">
             ...
             {task.due_date && (
                ...
                <div className="text-center px-2">
                    ...
                    <p className="font-black text-xs text-rose-500 tracking-tighter italic">{formatDate(task.due_date)}</p>
                </div>
                </>
             )}
          </div>
        ...
        {/* 3. ALIGNED GRID (Removed "Boring" Space) */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
          ...
          {/* RIGHT: ASSETS & CHECKPOINTS (Tightly Stacked) */}
               ...
               <div className="space-y-3">
                 {task.sub_tasks?.length === 0 ? (
                   ...
                   <div key={st.id} className="flex items-center gap-4">
                     <div className={`h-5 w-5 rounded-lg border-2 flex items-center justify-center transition-all ${st.is_completed ? 'bg-zinc-900 border-zinc-900 shadow-lg' : 'border-zinc-100'}`}>
      ...
      {viewingFile && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-10">
          ...
          <div className="relative w-full max-w-5xl h-full max-h-[85vh] bg-white rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col border border-white/10 animate-in zoom-in-95 duration-300">
